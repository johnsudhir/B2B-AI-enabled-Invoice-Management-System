...
const DeleteButton = (props) => {
  ...
  const Delete = () => {
    let len = props.selected.length;
    let ids = props.selected;
    for (let i = 0; i < len; i++) {
      axios
        .get(
          `http://localhost:8080/highRadiusFinal/DeleteInvoice?doc_id=${props.selected[i]}`
        )
        .then((response) => {
  ...
  return (
    <>
      ...
      <Button
        ...
        disabled={"true" ? props.selected.length === 0 : "false"}
      >
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Delete&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      </Button>

      {/*End*/}
      {/* <Button style={deleteTheme.but} onClick={handleClickOpen}>
        Delete
      </Button> */}
      <Dialog
        open={open}
      ...
      >
        ...
        <DialogActions style={{ backgroundColor: "#2A3E4C" }}>
          ...
          <Button onClick={Delete} style={deleteTheme.but1}>
            Delete
          </Button>
        </DialogActions>
      </Dialog>
